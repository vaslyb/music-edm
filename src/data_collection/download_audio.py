...
if __name__ == "__main__":
    # try:
        ...
        if not os.path.exists("./results/audio_2"):
            os.mkdir("./results/audio_2")
        count = 0
        for preview_url, ID, Genre in zip(preview_urls, IDs, Genres):
            if preview_url == "0" or preview_url == "nan" or Genre == "nan":
                print(count)
                count += 1
                continue
            ...
            if not os.path.exists("./results/audio_2/" + str(Genre)):
                os.mkdir("./results/audio_2/" + str(Genre))
            if not os.path.exists("./results/audio_2/" + str(Genre) + "/" + str(ID) + ".mp3"):
                os.system(
                    "wget -c "
                    + preview_url
                    + " -O "
